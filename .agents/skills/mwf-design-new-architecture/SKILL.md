...
2. Define the start state, durable end state, semantic oases, and Job Scope on
   ...
   durable result. Resolve waiting scope and interrupt declarations for every
   component. If partial reports are needed, apply the report interrupt branch
   guidance in `docs/architecture/graph.md`.
3. Define every node's role, runner, concurrency, main task, fallbacks,
   ...
   fallback system is hidden inside task code. Name the producing and receiving
   raw nodes for every managed input, and check producer-qualified writes
