...
- `runfrom START refuseafter STOP` keeps that full cleanup scope but stops new
  Hoeflein-component admission when STOP's component terminates.
- Fresh/destructive CLI commands clear affected trace journals unless
  `--keeptrace` is present; resume preserves the start component, clipboard
  snapshots preserve events, and recreated preserved jobs record origin changes.
