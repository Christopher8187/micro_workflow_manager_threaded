...
class SelectedExecutionOperation(ComponentExecutionOperation):
    """Retain selected roots, their Python values, and new causal work across repairs."""

    continues_after_nested_restarts = True
    live_admission = False
    preloaded_selection = True

    def __init__(self, workflow, node, jobs, context, *, scalar=False):
        super().__init__(workflow, context[1][node], context, None, None)
        self.roots = tuple(workflow.storage._read_session_job_roots(workflow.storage.db_connection(), context[0]))
        if [(member, job_id) for member, job_id, _ in self.roots] != [(node, job.job_id) for job in jobs]:
            raise RuntimeError('Selected execution requires its exact ordered admitted roots')
        self.root_jobs = {(node, job.job_id): job for job in jobs}
    ...
    def has_more_work(self):
        if self.stop_admission:
            return False
        observed = self.read_frontier()
        return any(status == 'queued' and (node, job_id) not in self.epoch_jobs for node, job_id, status in observed)
    ...
    def restart_job_ids(self, node):
        ...
        if replacements is not None:
            return replacements
        queued = {job_id for member, job_id, status in self.frontier if member == node and status == 'queued'}
        ordered = [job_id for member, job_id, _ in self.roots if member == node and job_id in queued]
    ...
    def finish_execution(self):
        self.frontier = self.read_frontier()
        unfinished = [(node, job_id) for node, job_id, status in self.frontier if status not in ('done', 'skipped')]
        if unfinished:
            if self.stop_admission:
                return
            node, job_id = unfinished[0]
            raise ComponentExecutionIncomplete(f'Selected execution has unfinished causal job {node}/{job_id}')
        self.workflow.refresh_component_status(set(self.component), allow_complete=True)
        self.workflow.storage.mark_selected_component_execution_ready(self.execution_context[0], self.completion)
        self.completed = True


def run_selected_execution(workflow, node, jobs, context, driver, *, scalar=False):
    operation = SelectedExecutionOperation(workflow, node, jobs, context, scalar=scalar)
    driver.drive(operation)
    results = [operation.values[(node, job.job_id)] for job in jobs]
