...
def read_preparation_footprint(storage, components, *, keep_trace=False):
    ...
    try:
        ...
        for row in connection.execute('SELECT receiver_node, relative_path FROM managed_input_files '
            ...
            matching = [owner for owner in claimants if owner['execution_id'] in selected_ids]
            if not matching:
                continue
            ...
            producer_components = {owner['component'] for owner in matching}
            if len(producer_components) != 1:
            ...
            component = selected_nodes.get(receiver, matching[0]['component'])
            inputs[component].append(PreparedInput(
                receiver, relative, tuple(sorted(claimants, key=lambda owner: owner['execution_id'])),
            ))
...
def validate_preparation_footprint(storage, connection, footprint):
    ...
    if _selected_owners(storage, connection, components) != footprint.owners:
