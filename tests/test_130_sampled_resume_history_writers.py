...
def _sampled_owner(storage, *, repair_job=False):
    ...
    _session(storage, 'sample-origin', 'interrupt', ('A',))
    ...
    _session(storage, 'resume-owner', 'main', ('A',))
...
def test_sampled_start_records_absent_history_and_failure_preserves_it(tmp_path):
...
def test_sampled_history_creation_rolls_back_with_failed_start(tmp_path, suppression):
    storage = FileStorage._create_new_project_state(tmp_path)
    try:
        snapshot = _sampled_owner(storage)
        raising = "RAISE(ABORT, 'start rejected')" if suppression == 'ABORT' else 'RAISE(IGNORE)'
        storage.submit_db_mutation(lambda connection: connection.execute(
            'CREATE TRIGGER reject_resume_start BEFORE UPDATE OF lifecycle ON component_states '
            f"WHEN NEW.lifecycle='running' BEGIN SELECT {raising}; END",
        ))
        before = _rows(storage)
        error = sqlite3.IntegrityError if suppression == 'ABORT' else RuntimeError
        message = 'start rejected' if suppression == 'ABORT' else 'Component changed before start'
...
def test_resume_preparation_rechecks_history_after_staging_and_restores_files(tmp_path, monkeypatch):
    storage = FileStorage._create_new_project_state(tmp_path)
    try:
        _sampled_owner(storage, repair_job=True)
        _seed_history(storage, 'sampled')
